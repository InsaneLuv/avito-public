...
async def httpx_client_proxied() -> AsyncGenerator[AsyncClient, None]:
    ...
    async with AsyncClient(
            proxy=f"http://{SQUID_PROXY_USER}:{SQUID_PROXY_PASSWORD}@{SQUID_PROXY_HOST}:{SQUID_PROXY_PORT}") as client:
        yield client
...
async def avito(client_id, client_secret):
    ...
    client = AvitoModels(client_id, client_secret)
...
async def openai(openai_api_token, httpx_client_proxied):
    ...
    return AsyncOpenAI(api_key=openai_api_token, http_client=httpx_client_proxied)
